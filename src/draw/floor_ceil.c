...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/04/27 21:49:39 by cedmulle          #+#    #+#             */
/*   Updated: 2024/04/27 23:29:05 by cedmulle         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static unsigned int	get_tex_color_fc(t_data *data, int x, int y, t_type type)
{
	...
	if (type == FLOOR)
		tex = get_tex(data->tex, FLOOR);
	else
		tex = get_tex(data->tex, CEIL);
...
static void	parse_n_draw_floor(t_data *data, int x, int y)
{
	...
	my_pixel_put(data->mlx, x, y, get_tex_color_fc(data, calc.x, calc.y, FLOOR));
}
...
void	draw_floor_sky(t_data *data, t_ray *ray, int x)
{
	...
	y = -1;
	floor = get_tex(data->tex, FLOOR);
	sky = get_tex(data->tex, CEIL);
